...
- **验证类型总览（按严格度）**：① CF Turnstile（最常见，扩展多能自动过）② 文本验证码（OPTICA，手输）③ hCaptcha（IOP 风控触发，最严，手点）④ reCAPTCHA（仅登录入口，管线内碰不到）
- **检索/平台型库（无 DOI 直达全文，不适用本管线）**：Web of Science、Scopus、EI Compendex、Inspec、JCR、InCites、ESI、SciFinder、MathSciNet、EDS、EBSCO、ProQuest、PQDT、DDS、NSTL、Total Materia 等——这些是"查目录/查指标"，找到目标后仍回到出版社/DOI 渠道下全文
